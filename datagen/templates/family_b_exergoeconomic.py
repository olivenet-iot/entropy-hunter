"""
EntropyHunter — Family B: Exergoeconomic (SPECO) Analysis Templates

Same physical parameters as Family A, plus economic parameters.
Asks the teacher model to perform SPECO thermoeconomic analysis.
"""
...
from economic_params import generate_economic_params


def _econ_block(econ: dict) -> str:
    """Format economic parameters block for prompts."""
    return f"""Economic data:
- Purchase equipment cost (PEC): €{econ['pec_eur']:,.0f}
- Installation factor: {econ['installation_factor']}
- Interest rate: {econ['interest_rate_pct']}%
- Equipment lifetime: {econ['lifetime_years']} years
- Maintenance cost factor: {econ['maintenance_factor_pct']}% of TCI/year
- Annual operating hours: {econ['annual_operating_hours']} h/year
- Energy cost ({econ['energy_type']}): {econ['energy_price_eur_kwh']} EUR/kWh"""


def _build_template(equipment_type: str, subtype_names: dict, params: dict,
                    operating_block: str) -> dict:
    """Generic template builder for exergoeconomic analysis."""
    name = subtype_names.get(params["subtype"], params["subtype"])
    econ = generate_economic_params(equipment_type, params["subtype"], params)

    prompt = f"""Perform a complete exergoeconomic (SPECO) analysis for a {name} {equipment_type.replace('_', ' ')}.
...
Provide step-by-step calculations ending with summary table and recommendations."""
    ...
    return {
        ...
        "metadata": {
            ...
            "analysis_type": "exergoeconomic",
            "operating_mode": params.get("operating_mode", "full_load"),
            "parameters": {k: v for k, v in params.items() if k != "_derived"},
            "economic_parameters": {k: v for k, v in econ.items() if k != "_derived"},
        }
    }


def template_exergoeconomic_compressor(params: dict) -> dict:
...
def template_exergoeconomic_boiler(params: dict) -> dict:
    ...
    fuel = fuel_names.get(params.get("fuel_type", ""), params.get("fuel_type", ""))
    block = f"""Operating conditions:
...
def template_exergoeconomic_heat_exchanger(params: dict) -> dict:
...
def template_exergoeconomic_chiller(params: dict) -> dict:
...
def template_exergoeconomic_pump(params: dict) -> dict:
...
def template_exergoeconomic_steam_turbine(params: dict) -> dict:
...
def template_exergoeconomic_dryer(params: dict) -> dict:
...
TEMPLATE_MAP_B = {
    "compressor":     (template_exergoeconomic_compressor, consistent_compressor_params),
    "boiler":         (template_exergoeconomic_boiler, consistent_boiler_params),
    "heat_exchanger": (template_exergoeconomic_heat_exchanger, consistent_heat_exchanger_params),
    "chiller":        (template_exergoeconomic_chiller, consistent_chiller_params),
    "pump":           (template_exergoeconomic_pump, consistent_pump_params),
    "steam_turbine":  (template_exergoeconomic_steam_turbine, consistent_steam_turbine_params),
    "dryer":          (template_exergoeconomic_dryer, consistent_dryer_params),
}


def generate_prompt(equipment_type: str, subtype: Optional[str] = None) -> dict:
    template_fn, params_fn = TEMPLATE_MAP_B[equipment_type]
