...
def check_example(example: dict) -> QualityResult:
    ...
    # Skip for hotspot_detection — multi-equipment table values don't form single balance
    ...
    if analysis_type == "hotspot_detection":
        checks["energy_balance"] = None  # skip for multi-equipment
    ...
    # Skip for hotspot_detection — multi-equipment analysis has per-equipment values
    # that don't match facility-level S_gen (known false positive since v0.1)
    if analysis_type == "hotspot_detection":
        checks["gouy_stodola"] = None  # skip, not applicable for multi-equipment
...
SCAFFOLD_KEY_MAP = {
    ...
    # === v0.4.1 RECOVERY: Exhaust/waste labels that were mismatching "exergy out" ===
    # Steam turbine exhaust (exact match prevents partial "exergy out" → exergy_out)
    ...
    # Boiler stack/shell losses (additional variants)
    ...
    # Heat exchanger waste (hot side residual / cold side excess)
